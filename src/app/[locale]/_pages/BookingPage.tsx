...
export default async function BookingPageContent({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const rawServices = await prisma.service.findMany({
    ...
    include: { translations: { where: { language: locale } }, category: { include: { translations: { where: { language: locale } } } } },
    orderBy: { price: 'asc' },
  });
  const services = rawServices.map(s => ({
    id: s.id, name: s.translations[0]?.name || 'İsimsiz Hizmet', price: s.price.toString(),
    duration: s.duration, categoryId: s.categoryId, categoryName: s.category.translations[0]?.name || 'Diğer',
  }));
  ...
  return (
    <main className="min-h-screen pt-32 pb-24 relative overflow-hidden flex items-center justify-center">
      ...
      <div className="w-full px-6 z-10">
        <BookingForm initialLocations={locations} initialServices={services} initialStaff={staffList} initialCustomer={initialCustomer} />
      </div>
    </main>
  );
}

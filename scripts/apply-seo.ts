...
async function updateDb() {
  console.log("DB Güncellemesi Başlıyor...");
  try {
    ...
    for (const page of pages) {
      for (const trans of page.translations) {
        ...
        if (d) {
          ...
          console.log(`[BAŞARILI] ${trans.slug} -> ${trans.language} güncellendi.`);
        }
      }
    }
    console.log("Tüm SEO/Görsel verileri başarıyla sisteme aktarıldı!");
  } catch (err) {
